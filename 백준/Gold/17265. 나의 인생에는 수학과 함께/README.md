...
- 일단 dfs문제이고 시작지점에서 도착 지점까지 모든 경우의 수를 브루트 포스로 구해서 모든 값 중 가장 큰값과 가장 작은 값을 찾아 내는것임
- 이때 가장 큰 문제는 어떻게 브루트 포스를 실행 할 것인가?
- 일단 dfs를 짤 때는 문제에서 오른쪽과 아래쪽으로 밖에 안가므로 x+1,y 와 x,y+1 만 해줌
- 이때 모든 경로를 탐색하기 위해서 먼저 한 경로를 탐색 한 후 들어온 경로의 방문체크를 없애고 거기서 다시 이어서 하면 된다고 생각함
- 이때 경로가 이미 가득 차 있으므로 방문기록을 없애는 동시에 pop을 실행해 경로도 같이 없애줌

## 알아야 할것

- dfs에서 브루트 포스를 위해서는 dfs가 끝난 후 방문기록을 없애는게 중요하다.
