...
- 처음에는 땅들을 다 라벨링해서 한 땅에서 다른 땅으로 지나가는 최소 거리를 찾으려함
- 그런데 그러면은 각각의 땅에서 땅으로 오는 과정에서 중간에 만나버림
- 그래서 한 땅의 물과 땅의 경계를 찾아서 거기서부터 다른 땅까지의 최소거리를 각각 구하고 그 중 최소값을 출력함 → 즉 모든 땅에 대해서 브루트포스로 햇다는 뜻
- 그런데 이건 pypy 말고 python3 가 훨씬 빠름

[문제 링크](https://www.acmicpc.net/problem/2146) 

### 성능 요약

메모리: 117968 KB, 시간: 208 ms
