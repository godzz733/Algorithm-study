...
## 큐의 정석
### 위치 정렬
먼저 덱을 쓸거라고는 생각함, 이 문제는 일단 나보다 큰 숫자들을 먼저 지우고 나보다 큰 숫자의 갯수를 센다 (여기까진 쉬움), 그리고 지금 위치를 보고 내가 원하는 숫자 앞에있는 나와 같은 숫자의 개수를 세야함. 그래서 덱을 변경할때마다 내 위치를 한칸씩 바꿔주어서 결과 덱에서 내 위치를 찾을 수 있게함 (이게 핵심)
이제 내 위치를 알고 내 앞의 나와 같은 숫자의 개수를 아니까 답을 알 수 있음
